...
def build_dummy_candles(n: int = 500) -> pd.DataFrame:
    rows = []
    base = 43000.0
...
def main() -> None:
    ...
    # Typical US30 synthetic point value for local logic test.
    point = 0.1
    df = enrich(df, cfg)
    idea = generate_trade_idea(df, cfg, point=point)
    ...
    print("can_trade:", can_trade, "reason:", reason)
